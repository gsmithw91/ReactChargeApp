...
function CarrierList() {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get('http://127.0.0.1:5000/react/eligibility/carriers')
            .then(response => {
    ...
    const handleCarrierClick = (carrierId) => {
        if (!insurancePlans[carrierId]) {
            axios.get(`http://127.0.0.1:5000/react/eligibility/insurance-plans/${carrierId}`)
                .then(response => {
                    setInsurancePlans(prevPlans => ({ ...prevPlans, [carrierId]: response.data }));
                })
                .catch(error => {
            ...
            {carriers.map(carrier => (
                <div key={carrier.CarrierID} className="carrier-row">
                    ...
                    <div className="insurance-plan-buttons">
                        {insurancePlans[carrier.CarrierID] && insurancePlans[carrier.CarrierID].map(plan => (
